...
class DropZone(QLabel):
    ...
    def __init__(self, client: LMStd) -> None:
        """
        ...
        try:
            ...
            self.setText(
                "Drag and Drop PDF File Here\n\n🔹 Action: Append Summary\n(Appends a short summary to the filename separated by '~')")
            self.setStyleSheet('''
                QLabel {
            ...
            if mime_data is not None and mime_data.hasUrls():
                ...
                if urls:
                    ...
                    if file_path.lower().endswith('.pdf'):
                        log_success(func_name, f"Valid PDF file dropped: {file_path}")
                        self.process_file(file_path)
                    else:
                        log_error(func_name, f"Dropped file is not a PDF: {file_path}")
        except Exception as e:
    ...
    def process_file(self, file_path: str) -> None:
        ...
        Args:
            file_path (str): The full path to the dropped PDF file.
        """
        ...
        if "~" in os.path.basename(file_path):
            log_step("Cycle", f"Skipping {os.path.basename(file_path)} as it already contains a '~'.")
            print_progress(1, 1, prefix='Dropped File Progress',
                           suffix='Skipped (already processed)', length=30)
            return
        ...
        try:
            log_step(func_name, "Attempting to extract text from dropped PDF.")
            ...
            if not text:
                ...
                log_success(func_name, f"Extracted {len(text)} characters of text.")
                log_step(func_name, "Proceeding to rename the PDF based on summary.")

                rename_success = rename_pdf_from_summary(self.client, file_path, text)
                if rename_success:
                    cycle_success = 1
                    log_success("Cycle", f"Completed processing for dropped file: {file_path}")
                else:
            ...
            print_summary_box("Cycle Summary (Interrupted)", 1, cycle_success, cycle_fail)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, client: LMStd) -> None:
        ...
        try:
            ...
            self.drop_zone = DropZone(client)
...
def main() -> None:
    ...
    try:
        ...
        log_success(func_name, "Application started successfully. Waiting for PDF drops...")
        sys.exit(app.exec_())
    except Exception as e:
